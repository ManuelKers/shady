...
public class IKLeg : MonoBehaviour
{
	...
	private Vector3 lastVectorToTargetPos = Vector3.zero;
	...
	void Start ()
	{
        ...
        maxDist = UpperLeg.GetComponent<Collider>().bounds.extents.x;// UpperLeg.transform.localScale.z + LowerLeg.transform.localScale.z;
	...
	void UpdateAngles ()
	{
		...
		float UpperLength = UpperLeg.transform.localScale.z;
		float LowerLength = LowerLeg.transform.localScale.z;
	...
	void LerpToNewPosition ()
	{
		...
		//We place the Lowerleg at the end of the Upperleg and pointing toward the target position
		LowerLeg.transform.position = UpperLeg.transform.position + UpperLeg.transform.forward * UpperLeg.transform.localScale.z;
